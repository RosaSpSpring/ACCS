...
public class MainActivity extends AppCompatActivity {
	...
	private List<CurData.DataBean> listByNode = new ArrayList<>();
	ArrayList<CurData.DataBean>  innerData	= new ArrayList<>();
	ArrayList<ArrayList<CurData.DataBean>> coll = new ArrayList<ArrayList<CurData.DataBean>>(  );
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < listByName.size(); i++) {
			...
			for (int j = 0; j <listByNode.size(); j++) {
				if (listByName.get( i ).getName().equals( listByNode.get( j ).getName() )){

				  innerData.add( i, listByNode.get( j ));
				 //coll怎么添加i 和 innerData联系起来最重要
				}
			}
		}
	...
	private void loadData() {
		...
		for (int i = 0; i < listByName.size(); i++) {
			...
			for (int k = 0; k < innerData.size(); k++) {
				TreeNode treeNode3 = new TreeNode(coll.get( i ).get( k ));
